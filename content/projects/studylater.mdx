...
La idea al desarrollar **StudyLater** es la de una aplicación donde se puedan descargar materiales de estudio, como PDFs, y tenerlos organizados y disponibles sin conexión. Tambien la intención fue probar implementar estas funcionalidades a traves de <CreateLink from="Capacitor"/>:
...
Una de las tareas más dificiles fue el descargar archivos. Para empezar, al utilizar la función `fetch()` para descargar archivos solian aparecer errores de *CORS*, sobre todo cuando las descargas provienen de sitios sin certificado SSL. Este comportamiento es el esperado dentro de JavaScript en un navegador, pero muchos de los PDFs que un usuario querria descargar están en FTPs públicos, sin HTTPS.
...
}}/>. Ahora la posibilidad de descargar desde HTTPS dependerá de la configuración de Android. Incluso este plugin ya incluye el método `download()`, y solo necesitaba saber en que ubicación almacenar los archivos. Esto se resuelve con el plugin <CreateLink from="Cordova" extend={{
  title: "File",
...
}}/>. Finalmente para que el *login* de Google se sienta más nativo, utilicé el plugin <CreateLink from="Cordova" extend={{
  title: "Google Plus",
...
Tambien probé la viabilidad de utilizar <CreateLink from="Capacitor"/> con otras tecnologías independientes de Ionic. Para construir las interfaces y darle vida a la aplicación utilicé  <CreateLink from="React"/>, asi como para el diseño utilicé <CreateLink from="Bootstrap"/>, aunque quizá no sea muy adecuado para una aplicación móvil, como si lo es para un sitio web responsivo.
...
- <CreateLink from="Ionic" extend={{
  title: "Ionic Native (plugins community)",
